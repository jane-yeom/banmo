...
// alert mock (window.alert이 JSDOM에서 에러 방지)
...
// 스텝2 진입 헬퍼: 닉네임 입력 + 전체 약관 동의 + 다음 클릭
function goToStep2(nickname = '테스터') {
  act(() => {
    // 닉네임 입력
    fireEvent.change(
      screen.getByPlaceholderText('사용할 닉네임을 입력해주세요'),
      { target: { value: nickname } }
    )
    // 개별 체크박스 각각 클릭 (onChange 트리거)
...
describe('신규 가입 - 스텝2 (유효한 닉네임)', () => {
  ...
  it('스텝2: 악기 선택 토글 (선택)', () => {
    ...
    expect(screen.getByText('피아노')).toHaveStyle({ background: '#F0EDE6' })
  })
  ...
  it('저장 버튼 클릭시 /users/me PATCH 호출', async () => {
    ...
    // nickname이 미리 설정된 user mock (useEffect가 이 값으로 초기화)
    useAuthStore.mockImplementation(() => ({
      user: { nickname: '반모테스터', profileImage: null },
      accessToken: 'mock-token',
      setAuth: jest.fn(),
      isLoggedIn: true,
    }))
    ...
    render(<WelcomePage />)
    goToStep2()  // 이미 '반모테스터' 닉네임으로 초기화됨

    const saveBtn = screen.getByText(/시작하기 🎵/)
    ...
    await waitFor(() => {
      expect(api.patch).toHaveBeenCalledWith(
        '/users/me',
        expect.objectContaining({ nickname: expect.any(String) })
      )
    })
  })
})
