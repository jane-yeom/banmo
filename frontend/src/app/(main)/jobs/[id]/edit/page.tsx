...
/** region 문자열을 province / district 로 분리 */
function parseRegion(region: string | null): { province: string; district: string } {
  if (!region) return { province: '', district: '' };
  for (const [prov, dists] of Object.entries(REGIONS)) {
    if (region === prov) return { province: prov, district: '' };
    if (dists.includes(region)) return { province: prov, district: region };
  }
...
export default function JobEditPage() {
  ...
  useEffect(() => {
    ...
    const { province: prov, district: dist } = parseRegion(post.region);
    ...
    setInitialized(true);
  }, [post, user, id, initialized, reset, router]);
  ...
  const handleImageFiles = async (files: File[]) => {
    ...
    if (toUpload.length === 0) {
      toast.error(`이미지는 최대 ${MAX_IMAGES}장까지 첨부 가능합니다.`);
      return;
    }
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div className="flex gap-3">
          <button
            ...
            className="flex-1 rounded-xl border-2 border-gray-200 py-4 text-base font-semibold text-gray-600 hover:bg-gray-50 transition-colors"
          >
            취소
          </button>
          <button
